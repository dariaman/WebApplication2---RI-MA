...
Public Class ProdukGroup
    ...
    Dim _ClsProdukGroup As New WebService.ClsProductGroup
        ...
        If Not UserLogin Is Nothing Then
            If Not Page.IsPostBack Then
                If UserLogin.IsActive Then
                    Try
                        Session("DashBoard") = "Product Group List<i class='fa fa-cube fa-fw'></i>"
                    ...
                    Catch ex As Exception
                        ...
                        Dim msg As String = String.Format("{0} - ProdukGroup - " & UserLogin.UserId & " - {1} - {2}{3}", Now.ToString("dd/MM/yyyy HH:mm:ss"), UserLogin.BranchCode, ex, Environment.NewLine)
                        WriteFile.Write(config.SetFullFilePath, msg)
                    End Try
                Else
                    Response.Redirect("login.aspx?p=ProdukGroup.aspx", False)
                End If
            End If
        Else
            Response.Redirect("login.aspx?p=ProdukGroup.aspx", False)
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click, LinkSubmit.Click
        Try
            ...
            If validasi() = True Then
                _ClsProdukGroup.typeProduk = txtTypeProduk.Text
                _ClsProdukGroup.DESCRIPTION = txtDESCRIPTION.Text
                _ClsProdukGroup.IsActive = chkAktiv.Checked
                _ClsProdukGroup.CRE_BY = UserLogin.UserId
                _ClsProdukGroup.CRE_IP = _sama.ClientIP_add

                If _ClsProdukGroup.InsertProduct() = True Then
    ...
    Public Function validasi() As Boolean
        If txtTypeProduk.Text = "" Or txtDESCRIPTION.Text = "" Then
    ...
    Protected Sub bindData(PRODUK As String)
        Try
            gridMenu.DataSource = _ClsProdukGroup.bindData(PRODUK, ddlSearch.SelectedValue)
            gridMenu.DataBind()
        Catch ex As Exception
    ...
    Protected Sub bindisiData(ID As String)
        Try
            Dim dt As DataTable = _ClsProdukGroup.bindisiData(ID)
            If dt.Rows.Count > 0 Then
                PnlMain.Visible = False
                pnlPopup.Visible = True
                txtTypeProduk.Text = dt.Rows(0)(0).ToString
    ...
    Private Sub DGuser_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gridMenu.RowCommand
        Try
            ...
            If e.CommandName = "UpdateLink" Then
                ...
                _sama.UpdateActive("MSProdukGroup", "ISACTIVE", IIf(sts = "True", "False", "True"), "TypeProduk", KEY)
                bindData(txtKeyWord1.Text)
            End If
        Catch ex As Exception
            ClientScript.RegisterStartupScript(Me.GetType, "confirm", "<script language=javascript>jqxAlert.Alert('Error!, Please contact IT Support');</script>")
